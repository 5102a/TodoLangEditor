...
function __asyncjs__js_transport_read_fully(){ return Asyncify.handleAsync(async () => { const payload = await new Promise((rs) => { function poll() { if (globalThis.debugPayload.length) { const payload = globalThis.debugPayload.shift(); if (payload) { try{ const br = JSON.parse(payload.slice(9)); if(br.breakpoints && !br.breakpoints.path.endsWith('index.js')){ return rs(''); } }catch{ } rs(payload); } } rs(''); } setTimeout(poll); }); if (payload === '') { return 0; } console.log("payload:",globalThis.debugPayload.length,payload); const len = (payload.length << 2) + 1; const ret = globalThis.wasmModule.stackAlloc(len); globalThis.wasmModule.stringToUTF8(payload, ret, len); return ret; }); }
