...
Module['ready'] = new Promise(function(resolve, reject) {
  ...
  readyPromiseReject = reject;
      ...
      if (!Object.getOwnPropertyDescriptor(Module['ready'], '_eval')) {
        Object.defineProperty(Module['ready'], '_eval', { configurable: true, get: function() { abort('You are getting _eval on the Promise object, instead of the instance. Use .then() to get called back with the instance, see the MODULARIZE docs in src/settings.js') } });
        Object.defineProperty(Module['ready'], '_eval', { configurable: true, set: function() { abort('You are setting _eval on the Promise object, instead of the instance. Use .then() to get called back with the instance, see the MODULARIZE docs in src/settings.js') } });
      }
...
var INITIAL_MEMORY = Module['INITIAL_MEMORY'] || 512557056;
...
assert(INITIAL_MEMORY == 512557056, 'Detected runtime INITIAL_MEMORY setting.  Use -s IMPORTED_MEMORY to define wasmMemory dynamically');
...
function createWasm() {
  ...
  function receiveInstance(instance, module) {
    ...
    //assert(wasmMemory.buffer.byteLength === 512557056);
...
function __asyncjs__js_transport_read_fully(){ return Asyncify.handleAsync(async () => { let count = 0; const payload = await new Promise((rs) => { function poll() { if (globalThis.debugPayload.length) { const payload = globalThis.debugPayload.shift(); if (payload) { try{ const br = JSON.parse(payload.slice(9)); if (!br.breakpoints.path.split('.js')[0].endsWith('index')) { setTimeout(poll, 20); return; } }catch{ } rs(payload); } } else if (++count >= 10) { rs(''); } else { setTimeout(poll, 20); } } setTimeout(poll, 20); }); count = 0; if (payload === '') { return 0; } console.log("payload:", payload, globalThis.debugPayload.length); const len = (payload.length << 2) + 1; const ret = globalThis.wasmModule.stackAlloc(len); globalThis.wasmModule.stringToUTF8(payload, ret, len); return ret; }); }
  ...
  function _cPostMessage(ctx, str) {
  ...
  var Asyncify = {State:{Normal:0,Unwinding:1,Rewinding:2,Disabled:3},state:0,StackSize:24576,currData:null,handleSleepReturnValue:0,exportCallStack:[],callStackNameToId:{},callStackIdToName:{},callStackId:0,asyncPromiseHandlers:null,sleepCallbacks:[],getCallStackId:function(funcName) {
        ...
        var ASYNCIFY_IMPORTS = ["env.do_wait","env.js_transport_read_fully","env.invoke_*","env.emscripten_sleep","env.emscripten_wget","env.emscripten_wget_data","env.emscripten_idb_load","env.emscripten_idb_store","env.emscripten_idb_delete","env.emscripten_idb_exists","env.emscripten_idb_load_blob","env.emscripten_idb_store_blob","env.SDL_Delay","env.emscripten_scan_registers","env.emscripten_lazy_load_code","env.emscripten_fiber_swap","wasi_snapshot_preview1.fd_sync","env.__wasi_fd_sync","env._emval_await","env._dlopen_js","env.__asyncjs__*"].map((x) => x.split('.')[1]);
...
var asmLibraryArg = {
  ...
  "__asyncjs__js_transport_read_fully": __asyncjs__js_transport_read_fully,
  "_localtime_js": __localtime_js,
  "_tzset_js": __tzset_js,
  "abort": _abort,
  "cPostMessage": _cPostMessage,
